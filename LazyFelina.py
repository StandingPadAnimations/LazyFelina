...
class Mist_OT_my_op(Operator):
    ...
    def execute(self, context):
        ...
        self.report({'ERROR'}, "Check Compositor, connect mist to map value and the image into the top input of MixRGB")
...
class advanceMist_OT_my_op(Operator):
    ...
    def execute(self, context):
        ...
        self.report({'ERROR'}, "Check Compositor, connect mist to map value and the image into the top input of screen MixRGB. Create a texture for texture node(clouds looks best but you do you).")
...
def sunbeams_comp_action(context):
    ...
    link(mix2_node.outputs[0], mix_node.inputs[0])
